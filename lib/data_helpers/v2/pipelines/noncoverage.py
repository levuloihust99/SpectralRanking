...
class NoncoverageFetchState(BaseModel):
    epoch: int
    iteration: int


class NoncoveragePipelineState(BaseModel):
    fetch_state: NoncoverageFetchState
    consumed: int
    buffer_index: int
...
class NoncoveragePipeline:
    ...
    def fetch(self):
        ...
        for item in items:
            ...
            item["type"] = PipelineType.NON_COVERAGE
    ...
    def set_state(self, state: NoncoveragePipelineState):
